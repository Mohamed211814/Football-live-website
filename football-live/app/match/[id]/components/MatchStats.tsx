    ...
    <div className="flex flex-col gap-5 py-4 px-2 md:px-8" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className={`flex justify-between items-center mb-6 px-4 ${isRTL ? '' : 'flex-row-reverse'}`}>
        <div className="flex flex-col items-center gap-2">
          <img src={statistics[0].team.logo} className="w-12 h-12 md:w-16 md:h-16 object-contain drop-shadow-md" alt={locale === 'ar' ? getArabicTeamName(statistics[0].team.name) : statistics[0].team.name} />
        ...
        <div className="flex flex-col items-center gap-2">
          <img src={statistics[1].team.logo} className="w-12 h-12 md:w-16 md:h-16 object-contain drop-shadow-md" alt={locale === 'ar' ? getArabicTeamName(statistics[1].team.name) : statistics[1].team.name} />
      ...
      {homeStats.map((stat, idx) => {
        ...
        return (
          <div key={idx} className="flex flex-col gap-2 group">
            ...
            {/* The progress bar */}
            <div className={`flex w-full h-2.5 bg-gray-100/80 rounded-full overflow-hidden shadow-inner gap-1 ${isRTL ? '' : 'flex-row-reverse'}`}>
