...
export default function MatchHeader({ match }: { match: Match }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl text-white p-6 md:p-8 mt-4 shadow-xl border border-red-900/20 bg-gradient-to-br from-[#8B1E1E] via-[#aa2222] to-[#4a0d0d]">
      ...
      <div className="relative z-10 flex justify-between items-center mb-8 border-b border-white/10 pb-4">
        <span className="text-sm md:text-base font-extrabold tracking-wide drop-shadow-sm opacity-95">{match.league}</span>
      ...
      <div className="relative z-10 flex justify-between items-center">
        ...
        <div className="flex flex-col items-center w-[30%]">
          <div className="relative w-20 h-20 md:w-28 md:h-28 mb-3 group">
            ...
            {match.homeLogo ? (
              <img src={match.homeLogo} alt={match.homeTeam} className="w-full h-full object-contain drop-shadow-2xl relative z-10 transform group-hover:scale-110 transition-transform duration-300" />
            ) : (
               <div className="w-full h-full bg-white/10 border border-white/20 rounded-full relative z-10"></div>
            )}
          </div>
          <span className="font-extrabold text-center text-sm md:text-xl drop-shadow-md leading-tight">{match.homeTeam}</span>
        </div>
        ...
        <div className="flex flex-col items-center w-[30%]">
          <div className="relative w-20 h-20 md:w-28 md:h-28 mb-3 group">
            <div className="absolute inset-0 bg-white/20 rounded-full blur-xl group-hover:bg-white/30 transition-all duration-300"></div>
            {match.awayLogo ? (
              <img src={match.awayLogo} alt={match.awayTeam} className="w-full h-full object-contain drop-shadow-2xl relative z-10 transform group-hover:scale-110 transition-transform duration-300" />
            ) : (
               <div className="w-full h-full bg-white/10 border border-white/20 rounded-full relative z-10"></div>
            )}
          </div>
          <span className="font-extrabold text-center text-sm md:text-xl drop-shadow-md leading-tight">{match.awayTeam}</span>
        </div>
      </div>
    </div>
  );
